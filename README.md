...
A comprehensive GB18030 encoding and decoding library implemented in MoonBit that provides robust support for Chinese character encoding standards, including complete Unicode mapping and efficient byte sequence processing.

## 🚀 Key Features

• 🎯 **Core Encoding/Decoding** – Complete GB18030 to Unicode and Unicode to GB18030 conversion  
...
• 📊 **Performance Optimized** – Efficient byte sequence processing and memory management  
• 🔄 **Error Handling** – Comprehensive error detection and recovery mechanisms  
• 🎨 **Flexible APIs** – Multiple encoding/decoding interfaces for different use cases  
• 📈 **Statistics & Analysis** – Character counting and Chinese character detection  
• 🔍 **Type Safety** – Robust type system with comprehensive error types  
• 📦 **Batch Operations** – Efficiently handle large text data  
...
match encoded_result {
    Ok(bytes) => {
        ...
        // byte_count will be the total number of bytes needed
    }
    Err(_) => {
...
// Get encoding length without actual encoding
let length = get_gb18030_length("Hello 你好")
// length will be the number of bytes needed for encoding
```
...
Analyze text content and get statistics.
...
### 🎨 Unicode Mapping
Work with Unicode to GB18030 mappings.
...
### Utility Functions
- `get_gb18030_length(s)` - Get encoding length for string
...
4. **Byte Sequence Generation**: Generate proper GB18030 byte sequences
...
- **Incomplete Sequences**: Handle truncated byte sequences gracefully
- **Out of Range**: Validate byte ranges and provide fallback values
- **Mapping Failures**: Use default values for unmapped characters

## 🧪 Test Coverage

The project includes comprehensive test cases with **85.61% code coverage**:
...
- **Error Path Coverage**: Comprehensive error handling tested
- **Boundary Condition Coverage**: Edge cases and limits tested
...
GB18030 is the official Chinese character encoding standard that supports:
...
- **Byte sequences**: 1 byte for ASCII, 2 bytes for Chinese characters
